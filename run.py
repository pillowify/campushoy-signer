...
def get_sign_task_info(user):
    ...
    datas = json.loads(requests.post(url=SIGN_TASK_URL,
                       json={}, headers=header).text)['datas']
    un_signed_tasks = datas['unSignedTasks']
    leave_tasks = datas['leaveTasks']
    for task in un_signed_tasks + leave_tasks:
...
def preprocess(cookie):
    ...
    user['deviceId'] = config['user']['deviceId']
    user['cookie'] = cookie
    sign_task_info = get_sign_task_info(user)
    if sign_task_info['signWid'] == -1:
        return -1
    form = get_req_form(user, get_answer_form(
        user, get_detail_task(user, sign_task_info)))
...
def submit_task(form):
    res_body = json.loads(requests.post(
        url=SUBMIT_SIGN_URL, json=form, headers=lite_header).text)
    return res_body['message']


def run(cookie, max_times=5, delay=0):
    form = preprocess(cookie)
    if form == -1:
        utils.print_log('无签到任务')
        return
    utils.print_log('已生成提交信息')
    flag = delay > 0
    while delay > 0:
        utils.print_log('即将在 ' + str(delay) + ' 秒后提交', end='\r')
        delay -= 1
        time.sleep(1)
    if flag:
        utils.print_log('计时器结束')
    count = 1
    while count <= max_times:
        utils.print_log('正在尝试第 ' + str(count) + ' 次提交 ...')
        res = submit_task(form)
        if res == 'SUCCESS':
            utils.print_log('已完成')
            break
        else:
