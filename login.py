...
def get_cookie():
    ...
    res = requests.get(LOGIN_URL, headers={
                       'User-Agent': USER_AGENT}, allow_redirects=False)
    if res.status_code == 418:
        utils.print_log('请求被拦截')
        return -1
    cookie = res.cookies
    redirect_url = res.headers['Location']
    _2lBepC = urllib.parse.parse_qs(
        urllib.parse.urlparse(redirect_url).query)['_2lBepC']
    res = requests.post(LT_URL, headers={
                        'User-Agent': USER_AGENT}, cookies=cookie, data={'lt': _2lBepC})
    ...
    if len(res.cookies) == 0:
        utils.print_log('用户名或密码错误')
        return 0

    redirect_url = res.headers['Location']
    cookie.update(res.cookies)
    res = requests.post(redirect_url, headers={
                        'User-Agent': USER_AGENT}, cookies=cookie, allow_redirects=False)
    cookie.update(res.cookies)
    cookie_str = ''
    for item in cookie:
        cookie_str = cookie_str + item.name + '=' + item.value + ';'
    utils.print_log('已获取Cookie')
    return cookie_str
